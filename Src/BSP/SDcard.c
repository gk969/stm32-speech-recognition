...
#define DeviceMode SD_DMA_MODE    	 
/*                                 	  
#define DeviceMode SD_INTERRUPT_MODE 
#define DeviceMode SD_POLLING_MODE   
*/

vu32	CardSize_B=0;
#define	CardSize_KB 	(CardSize_B>>10)
#define	CardSize_MB 	(CardSize_B>>20)

static u32 CardType =  SDIO_STD_CAPACITY_SD_CARD_V1_1;
static u32 RCA = 0;
static u16 Erase_Cheak=0;

#if DeviceMode!=SD_DMA_MODE
static u32 TotalNumberOfBytes = 0;
#endif

static u32 StopCondition = 0;
...
{
	SD_Error Init_Status = SD_OK;

   	SD_LowLevel_Init();
	
	SDIO_DeInit();
	
	Init_Status = SD_PowerON();
	if (Init_Status != SD_OK)
	{	
		return Init_Status;
	}
	//USART1_printf("PowerON OK! ");
	 
	Init_Status=SD_InitializeCards();
	if (Init_Status != SD_OK)
	{
		/* CMD Response TimeOut (wait for CMDSENT flag) */
		return(Init_Status);
	}
	USART1_printf("Card Size:%dMB",CardSize_MB);
	...
	Init_Status = SD_SelectDeselect((u32) (RCA << 16));
	/**/
	if (Init_Status == SD_OK)
	{
		Init_Status = SD_EnableWideBusOperation(SDIO_BusWide_4b);
		if(Init_Status==SD_OK)
		{
			USART1_printf("SDCard init OK!");
		}
	}
...
SD_Error SD_InitializeCards(void)
{
  SD_Error errorstatus = SD_OK;
  u16 rca = 0x01;
  u8 tmp=0,RdBlockLen=0,DeviceSizeMul=0;
  u16 DeviceSize=0;
  u32 CSD_Tab[4];
  ...
  if ((CardType == SDIO_STD_CAPACITY_SD_CARD_V1_1) || (CardType == SDIO_STD_CAPACITY_SD_CARD_V2_0))
  {
    ...
    DeviceSizeMul = (tmp & 0x03) << 1;
    
    tmp = (u8)((CSD_Tab[2] & 0x0000FF00) >> 8);
    DeviceSizeMul |= (tmp & 0x80) >> 7;
  }
  ...
  CardSize_B=(1<<(RdBlockLen+DeviceSizeMul+2))*DeviceSize;
...
SD_Error SD_ReadMultiBlocks(u32 addr, u32 *readbuff, u16 BlockSize, u32 NumberOfBlocks)
{
  ...
  if ((addr+BlockSize*NumberOfBlocks)>=CardSize_B)
  {
    errorstatus = SD_INVALID_PARAMETER;
    return(errorstatus);
  }
...
#if DeviceMode == SD_POLLING_MODE
  u32 *tempbuff = writebuff , bytestransferred = 0;
...
SD_Error SD_Erase(u32 startaddr, u32 endaddr)
{
  ...
  if ((Erase_Cheak & SD_CCCC_ERASE) == 0)
  {
    errorstatus = SD_REQUEST_NOT_APPLICABLE;
    return(errorstatus);
  }
