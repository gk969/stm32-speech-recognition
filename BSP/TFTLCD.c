...
void LCD_DrawLine(u16 left1, u16 top1, u16 left2, u16 top2,u16 color)
{
	...
	else if(Abs_Value(top2,top1)>Abs_Value(left2,left1)) 
	{
		if(top1>top2)
		{
			t=left1;left1=left2;left2=t;
			t=top1;top1=top2;top2=t;
		}
...
{       
	...
	Addr=72*((*(cn+1)-0xA1)+(*cn-0xA1)*94);
	Flash_Read(Addr,Font_Tab,72);
	...
	for(row=0;row<24;row++)
	{
		for(rank=0;rank<3;rank++)
		{
			for(bit=0;bit<8;bit++)
			{
				if((*(Font_Tab+row*3+rank)&(1<<(7-bit)))!=0)
				{
